...
# Se JÁ está logado, libera o sistema completo
    ...
    # =========================================================
    # TELA 1: DASHBOARD DE VENDAS (COM FINANCEIRO E EXPORTAÇÃO)
    # =========================================================
    if menu == "📈 Dashboard de Vendas":
        ...
        try:
            ...
            # Puxa o histórico de movimentações para calcular o faturamento e lucro real
            query_vendas = '''
        ...
        if not df.empty:
            # --- CÁLCULOS FINANCEIROS (IDEIA 01) ---
            # 1. Capital Investido: O custo de tudo que está parado no estoque hoje
            capital_investido = (df['Qtd Atual'] * df['Preço Custo']).sum()
            
            # 2. Faturamento Bruto e Lucro Real (Baseado nas saídas/vendas reais)
            ...
            # --- BLOCO DE MÉTRICAS VISUAIS ---
            st.subheader("💰 Resumo Financeiro e Operacional")
            col_fat, col_lucro, col_inv, col_alertas = st.columns(4)
            
            col_fat.metric("Faturamento Total", f"{faturamento_total:R$ ,.2f}")
            col_lucro.metric("Lucro Líquido Real", f"{lucro_real:R$ ,.2f}")
            col_inv.metric("Capital em Estoque", f"{capital_investido:R$ ,.2f}")
            ...
            # --- TABELA DE PRODUTOS COM OPÇÃO DE EXPORTAR (IDEIA 03) ---
            st.markdown("---")
            ...
            with col_botao:
                # Prepara o arquivo CSV formatado com ponto e vírgula e encoding para o Excel abrir sem bugar acentos
                csv_dados = df.to_csv(index=False, sep=';', encoding='utf-8-sig')
                st.download_button(
        ...
        with st.form("form_cadastro"):
            ...
            if botao_cadastrar:
                if nome:
                    ...
                    st.warning("O nome do product é obrigatório.")
    ...
    # =========================================================
    # TELA 4: HISTÓRICO DE AUDITORIA (COM BOTAO EXPORTAR TAMBÉM!)
    # =========================================================
        ...
        if not df_historico.empty:
            # Também adicionamos um botão de exportar no Histórico de Auditoria!
