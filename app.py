...
# Se JÁ está logado, libera o sistema completo
    ...
    if menu == "📈 Dashboard de Vendas":
        ...
        if not df.empty:
            ...
            # --- TABELA DE PRODUTOS COM EXPORTAÇÃO EXCEL REAL (.XLSX) ---
            st.markdown("---")
            ...
            with col_botao:
                # Criação do arquivo Excel legítimo usando o motor 'xlsxwriter' que já vem na nuvem
                buffer = io.BytesIO()
                ...
                st.download_button(
                    ...
                    file_name="relatorio_estoque_atual.xlsx",  # <-- Extensão oficial do Excel (.xlsx)
    ...
    # =========================================================
        ...
        if not df_historico.empty:
            # Também atualizado para gerar planilha oficial do Excel no histórico!
            buffer_hist = io.BytesIO()
            with pd.ExcelWriter(buffer_hist, engine='xlsxwriter') as writer_hist:
                df_historico.to_excel(writer_hist, index=False, sheet_name='Histórico de Movimentações')
