...
# Se JÁ está logado, libera o sistema completo
    ...
    if menu == "📈 Dashboard de Vendas":
        ...
        if not df.empty:
            ...
            # --- TABELA DE PRODUTOS COM OPÇÃO DE EXPORTAR COMPATÍVEL ---
            st.markdown("---")
            ...
            with col_botao:
                # Mudança estratégica: exportação direta em CSV com delimitador Excel e BOM de encoding
                # Isso impede o aviso de segurança e separa as colunas perfeitamente direto pelo Pandas
                csv_excel = df.to_csv(index=False, sep=';', encoding='utf-8-sig')
                st.download_button(
