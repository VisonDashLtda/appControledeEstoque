...
# Se JÁ está logado, libera o sistema completo
    ...
    if menu == "📈 Dashboard de Vendas":
        ...
        try:
            conn = conectar_banco()
            # Trocado de "ID" para "Código" para evitar permanentemente o bug SYLK do Excel
        ...
        if not df.empty:
            ...
            if total_criticos > 0:
                st.error(f"⚠️ **Atenção:** Você tem {total_criticos} product(s) abaixo ou no limite do estoque mínimo!")
            ...
            # --- TABELA DE PRODUTOS COM OPÇÃO DE EXPORTAR (CSV BLINDADO) ---
            st.markdown("---")
            ...
            with col_botao:
                # Gerando o CSV puro usando ponto e vírgula e o UTF-8-SIG (padrão do Excel no Brasil)
