...
# Se JÁ está logado, libera o sistema completo
    ...
    if menu == "📈 Dashboard de Vendas":
        ...
        if not df.empty:
            ...
            # --- BLOCO DE MÉTRICAS VISUAIS (CORRIGIDO) ---
            st.subheader("💰 Resumo Financeiro e Operacional")
            col_fat, col_lucro, col_inv, col_alertas = st.columns(4)
            
            # Correção aqui: O 'R$' vai antes da formatação numérica do Python
           ...
           # --- TABELA DE PRODUTOS COM OPÇÃO DE EXPORTAR (AGORA EM EXCEL NATIVO) ---
            st.markdown("---")
            ...
            with col_botao:
                # Cria o arquivo Excel real em memória binária usando uma função do Pandas
                import io
                buffer = io.BytesIO()
                ...
                st.download_button(
                    ...
                    file_name="relatorio_estoque_atual.xlsx",  # <-- Extensão oficial do Excel
